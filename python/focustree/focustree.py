...
def extract_selected_leaves(tree: Node) -> list[Node]:
    focused_node_slice = []
...
def get_leaves(node: Node, leaf_slice: list[Node]):
    if node.get_flag() == Flag.SKIP:
        return
    for child in node.get_children():
        get_leaves(child, leaf_slice)
    if node.is_leaf():
        leaf_slice.append(node)
